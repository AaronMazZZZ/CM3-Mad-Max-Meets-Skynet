...
class RacecarController(Controller):
    def __init__(self, trajectory, simulation_data, control_constants, speed_limits):
        ...
        print("create RacecarController")
    
    #获取机器人的下一个参考位置
    def set_next_reference(self):
        current_point = Point()
        current_point.x = self.x_n
        current_point.y = self.y_n
        
        reference = self.trajectory.get_position_at(self.i+1, current_point)
    ...
    def compute_control_actions(self, pose, twist, i):
        ...
        #设置当前参考（没有实际作用）
        self.set_current_reference(self.trajectory.get_position_at(self.i, current_point))
    ...
    def compute_linear_speed_reference(self):
        #根据下一个参考位置和机器人当前位置还有delta时间，计算参考线性速度
        #如果点之间的距离是给定的，那么点之间的距离是过大的话会导致速度很大
